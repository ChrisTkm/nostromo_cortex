...
import { clampAutoRefreshSeconds } from "./autoRefresh";
...
describe("clampAutoRefreshSeconds", () => {
  ...
  it("1000 -> 600 (clamp max)", () =>
    expect(clampAutoRefreshSeconds(1000)).toBe(600));
  it("NaN -> 0", () => expect(clampAutoRefreshSeconds(NaN)).toBe(0));
  it("undefined -> 0", () =>
    expect(clampAutoRefreshSeconds(undefined)).toBe(0));
  it("string numeric '15' -> 15", () =>
    expect(clampAutoRefreshSeconds("15")).toBe(15));
});
