/* * BORRAR — huérfano tras migración atómica de graph. Ya nadie lo importa
   (el viejo Toolbar era su único consumidor). Borrar también sus estilos
   `.hud-corner*` en webview/styles.css. */
...
export function HudCorner(props: {
  ...
  return (
    <div className="hud-corner hud-corner--inline">
      ...
      {props.planContext ? (
        <div
          className="hud-corner__ring-wrap"
          title={`${done}/${total} tasks done in ${props.planContext.code}`}
        >
          <svg
            className="hud-corner__ring"
            viewBox="0 0 36 36"
            aria-hidden="true"
          >
            <circle
              className="hud-corner__ring-track"
              cx="18"
              cy="18"
              r={RING_RADIUS}
            />
          ...
          <div className="hud-corner__ring-text">{progressPercent}%</div>
        </div>
      ) : null}
    </div>
  );
