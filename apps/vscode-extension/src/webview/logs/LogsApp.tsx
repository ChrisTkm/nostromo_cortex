          ...
          {emptyState === "empty" ? (
            <div className="logs-empty-state">
              ...
              <h2 className="logs-empty-state__title">
                No logs available in the current collection.
              </h2>
              <p className="logs-empty-state__text">
                Refresh the panel or verify the extension is pointed at the
            ...
            groupedLogs.map((group) => (
              <section
                className={`logs-execution-group${group.isUngrouped ? " logs-execution-group--ungrouped" : ""}`}
                key={group.id}
              >
                <button
        ...
        <aside className="logs-detail">
          ...
          <div className="logs-detail__meta">
            ...
            {selectedLog.executionId ? (
              <span className="log-chip">{selectedLog.executionId}</span>
            ) : null}
            {selectedLog.tag ? (
              ...
              <div className="logs-detail__fields">
                {selectedLog.details.map((detail) => (
                  <div className="logs-detail__field" key={detail.key}>
                    ...
                    <div className="logs-detail__field-value">
                      {detail.value}
                    </div>
                  </div>
                ))}
